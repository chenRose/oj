...
int ptrace_program(int pid,OJ_STATUS *oj_status,struct rusage*);
...
int compile(task_t *task,OJ_STATUS *oj_status)
{
	...
	else
	{
		...
		int status = ptrace_program(pid,oj_status,&resource);
		if(*oj_status==OJ_AC)
		{
			if(status==0)
			{
...
int ptrace_program(int pid,OJ_STATUS *oj_status,struct rusage * resource)
{
	int status;
	...
	while(1)
	{
		...
		if(WIFEXITED(status))
		{
			...
			log_notice("用户CPU:%ld(%ld),系统CPU时间：%ld(%ld)内存使用量:%ld",resource->ru_utime.tv_sec,
			resource->ru_utime.tv_usec,resource->ru_stime.tv_sec,resource->ru_stime.tv_usec,resource->ru_maxrss);
...
int handler_work(void * r_task)
{
	...
	if(status != 0)
	{
		//编译失败，可以直接返回，而不用接着执行
		oj_status=OJ_CE;
		strncpy(task->result,str_oj_status[oj_status],sizeof(task->result));
		return -1;
		//todo:把结果写到数据库中		
	}
	else
	{
		...
		else
		{
			...
			//调用沙箱来监听
			struct rusage resource;
			ptrace_program(pid,&oj_status,&resource);		
			//程序返回，开始比较结果
			if(oj_status==OJ_AC)
			{
				//程序正常运行，开始比较输出
				
				int iRet = compare_files("../test.out",OUT);
				if(iRet != 0)
				{
					//如果输出结果不正确，则将状态设置为错误答案
					oj_status=OJ_WA;					
				}
				//否则，此时的oj_status状态还是AC
			}
			else
			{				
			...
			strncpy(task->result,str_oj_status[oj_status],sizeof(task->result));
			//保存运行时间ms为单位
			task->use_time = resource.ru_utime.tv_usec/1000+resource.ru_utime.tv_sec*1000 + //用户CPU时间
							 resource.ru_stime.tv_usec/1000+resource.ru_stime.tv_sec*1000;
			//保存运行所用内存：这里与编译时做比较
			if(task->use_memory < resource.ru_maxrss)
			{
				task->use_memory=resource.ru_maxrss;
			}
			if(oj_status==OJ_AC)
			{
				task->score=100;
			...
			//现在把结果发送给消息队列，由master统一写入数据库Mysql
			mymsg_t msg;
			msg.mtype=1;
			int msg_len =sizeof(task_t);
			memcpy(&msg.task,task,msg_len);
			log_notice("send to master...[%d]",g_handler->master_msg_id);
			log_notice_s("bB","--------------------");
			...
			int iRet = msgsnd(g_handler->master_msg_id,&msg,sizeof(msg.task),0);
			if(iRet == -1)
				{
					log_error("msgsnd error. errno :%d",errno);
					log_notice("%d,%d",g_handler->master_msg_id,msg_len);
	...
	log_notice("task 处理完成:%s/%s，结果为:%s",task->dir,task->source,str_oj_status[oj_status]);
	chdir(old_dir);
	log_notice("[信号量值为：%d]\n",sem_get(g_handler->sem_id));
	log_error("---------------------------------------------------------------");
	free(task);
